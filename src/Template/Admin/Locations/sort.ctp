<?php
  $this->layout = 'admin';
  $this->assign('title', __d('CakeLocation', 'Location').'並び替え - '.__d('CakeLocation', 'Website Admin Title').' | '.__d('CakeLocation', 'Website Title'));
...
<ol class="breadcrumb">
  ...
  <li class="breadcrumb-item active" aria-current="page"><?= __d('CakeLocation', 'Location') ?>並び替え</li>
</ol>

<div class="container">
  <div class="row align-items-end mb-2">
    <div class="col-md">
      <h2><?= __d('CakeLocation', 'Location') ?>並び替え<hr class="d-none d-md-block"></h2>
    </div>
    ...
    <div class="card-body">
      ...
      <ul id="sortList" class="list-group">
        ...
        <?php foreach ($locations as $location) : ?>
          <?php $count += 1; ?>
          <li data-id="<?= h($location->id) ?>" data-no="<?= $count ?>" class="list-group-item">
            <?= h($location->name) ?>
          </li>
        <?php endforeach; ?>
      </ul>
